...
export default function HistoryScreen({ navigation, route }) {
  ...
  // Helper function to get user-specific storage keys (same as CompareScreen)
  ...
  // Handle autoOpenUrl from navigation params (when coming from "Already Scanned" card)
  useEffect(() => {
    const autoOpenUrl = route?.params?.autoOpenUrl;
    if (autoOpenUrl && scans.length > 0 && !loading) {
      // Normalize URLs for comparison (same as web app)
      ...
      const targetUrl = normalizeForMatch(autoOpenUrl);
      const matchingScan = scans.find(scan => {
        if (!scan.listing_url) return false;
        return normalizeForMatch(scan.listing_url) === targetUrl;
      });
      
      if (matchingScan) {
        // Clear the route param to prevent re-triggering
        navigation.setParams({ autoOpenUrl: undefined });
        // Open the matching scan - handleScanPress is defined later but accessible via closure
        handleScanPress(matchingScan);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [scans, loading, route?.params?.autoOpenUrl, navigation]);
