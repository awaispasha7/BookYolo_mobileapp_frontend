...
const CompareScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    if (currentUserId && lastUserIdRef.current !== null && lastUserIdRef.current !== currentUserId) {
      // User changed - clear old local storage data
      const clearOldData = async () => {
        try {
          // Clear old format keys (for backward compatibility cleanup)
          await AsyncStorage.removeItem('local_comparisons');
          await AsyncStorage.removeItem('compare_chats');
  ...
  const loadCompareChats = async () => {
    try {
      const storageKey = getUserStorageKey('compare_chats');
      const savedCompareChats = JSON.parse(await AsyncStorage.getItem(storageKey) || '[]');
  ...
  const loadCurrentCompareChat = async () => {
    try {
      ...
      const currentChatKey = getUserStorageKey('current_compare_chat_id');
      const currentChatId = await AsyncStorage.getItem(currentChatKey);
      if (currentChatId) {
        ...
        const refreshChatMessages = async () => {
          try {
            ...
            if (!chatError && chatData && chatData.messages) {
              ...
              // Sort messages by timestamp to ensure chronological order (oldest first, newest at bottom)
  ...
  const storeComparisonLocally = async (firstListing, secondListing, answer, question) => {
    try {
      ...
      // Store initial messages array for chat history
      messages: [
        {
          role: "user",
          content: question || "Compare these listings",
          messageType: "compare",
    ...
    // console.log('💾 STORING: Created comparison object:', comparison);
      ...
      // Get existing local comparisons (user-specific)
      ...
      await AsyncStorage.setItem(storageKey, JSON.stringify(limitedComparisons));
  ...
  // Save new message to local compare chat history
  const saveLocalCompareMessage = async (compareId, role, content) => {
    try {
      const storageKey = getUserStorageKey('local_comparisons');
      const localComparisons = await AsyncStorage.getItem(storageKey);
      if (!localComparisons) return;
      
      const comparisons = JSON.parse(localComparisons);
      const compareIndex = comparisons.findIndex(c => c.id === compareId);
      
      if (compareIndex !== -1) {
        // Initialize messages array if it doesn't exist
        if (!comparisons[compareIndex].messages) {
          comparisons[compareIndex].messages = [];
        }
        
        // Add new message
        comparisons[compareIndex].messages.push({
          role: role,
          content: content,
          created_at: new Date().toISOString()
        });
        ...
        await AsyncStorage.setItem(storageKey, JSON.stringify(comparisons));
        console.log('✅ [DEBUG] Saved message to local compare:', compareId);
      }
    } catch (error) {
      console.error('❌ [DEBUG] Error saving local compare message:', error);
    }
  };
  ...
  const saveCompareChat = async (compareChat) => {
    try {
      ...
      await AsyncStorage.setItem(storageKey, JSON.stringify(newCompareChats));
      
      // Save current chat ID (same as web frontend) - user-specific
      const currentChatKey = getUserStorageKey('current_compare_chat_id');
      await AsyncStorage.setItem(currentChatKey, compareChat.id);
  ...
  const handleRecentComparePress = React.useCallback(async (compare) => {
    try {
      // Set loading state to show loading indicator instead of blank screen
      setLoadingCompare(true);
      // Clear messages to prevent showing old content during loading
      setMessages([]);
      ...
      if (compare.source === 'backend' && compare.chatId) {
        ...
        if (chatData && chatData.chat) {
          ...
          // Use fallback scan data if compareScanData is not available (for message processing)
          const finalScanData = compareScanData || (fallbackScan1 && fallbackScan2 ? {
            scan1: fallbackScan1,
            scan2: fallbackScan2
          } : null);
          
          // DEBUG: Log scan data availability
          console.log('🔍 [DEBUG] finalScanData:', finalScanData ? {
            hasScan1: !!finalScanData.scan1,
            hasScan2: !!finalScanData.scan2,
            scan1Url: finalScanData.scan1?.listing_url || finalScanData.scan1?.url,
            scan2Url: finalScanData.scan2?.listing_url || finalScanData.scan2?.url
          } : 'NULL');
          
          // Process messages and format them for display (matching web app loadChat)
          // Ensure all user messages (AI questions) are included
          const filteredMessages = (chatData.messages || [])
            .filter((msg) => {
              // Filter out "Rating" messages (only assistant messages, not user questions)
              if (msg.role === 'assistant' && msg.content && msg.content.trim().toLowerCase() === 'rating') {
                return false;
              }
          ...
          // Track if we've seen the first assistant message
          ...
          const processedMessages = filteredMessages.map((msg, index) => {
              ...
              if (msg.role === 'assistant' && chatData.chat.type === 'compare') {
                ...
                const hasComparisonContent = msg.content && (msg.content.includes('Listing A:') || msg.content.includes('Comparative Analysis:'));
                ...
                if (hasComparisonContent) {
                  ...
                  if (finalScanData) {
                    message.comparedScans = finalScanData;
                ...
                } else if (finalScanData && !firstAssistantMessageSeen && msg.content && msg.content.trim().length > 0) {
                  // If this is the first assistant message and we have scan data, treat it as comparison
                  // This handles cases where the comparison message doesn't have the expected format
                  message.isComparison = true;
                  message.comparedScans = finalScanData;
                  // Keep the original content as the analysis
          ...
          // If we have scan data but no comparison message in processed messages, attach it to the first assistant message
          const hasComparisonMessage = processedMessages.some(msg => msg.isComparison && msg.comparedScans);
          if (!hasComparisonMessage && finalScanData && finalScanData.scan1 && finalScanData.scan2) {
            // Find the first assistant message that might be the comparison result (excluding follow-up)
            const firstAssistantMessage = processedMessages.find(msg => 
              msg.role === 'assistant' && 
              msg.content && 
              msg.content.trim().length > 0 &&
              !msg.content.includes("Do you have any question")
            );
            
            if (firstAssistantMessage) {
              // Attach scan data to the first assistant message
              firstAssistantMessage.isComparison = true;
              firstAssistantMessage.comparedScans = finalScanData;
              // If content is empty or just whitespace, add default content
              if (!firstAssistantMessage.content || firstAssistantMessage.content.trim().length === 0) {
                firstAssistantMessage.content = "Comparative Analysis:";
              }
            } else {
              // Create a new comparison message from scan data if no assistant message exists
              // Insert it before the follow-up message if there is one
              const followUpIndex = processedMessages.findIndex(msg => 
                msg.content && msg.content.includes("Do you have any question")
              );
              ...
              const comparisonMessage = {
                ...
                isComparison: true,
                comparedScans: finalScanData,
                timestamp: chatData.chat.created_at || new Date().toISOString()
              };
              
              if (followUpIndex >= 0) {
                processedMessages.splice(followUpIndex, 0, comparisonMessage);
              } else {
                processedMessages.unshift(comparisonMessage);
              }
            }
          }
          
          // Final check: if we still don't have a comparison message but have scan data, force create one
          const stillNoComparison = !processedMessages.some(msg => msg.isComparison && msg.comparedScans);
          if (stillNoComparison && finalScanData && finalScanData.scan1 && finalScanData.scan2) {
            // Force create comparison message at the beginning
          ...
          // Always add follow-up prompt message (same as local compares)
          // This ensures there's always at least one message to display
          ...
          // CRITICAL: Always ensure we have a valid comparison message with scan data
          // This is the most important step to prevent blank screen
          const hasValidComparison = processedMessages.some(msg => 
            msg.isComparison && 
            msg.comparedScans && 
            msg.comparedScans.scan1 && 
            msg.comparedScans.scan2 &&
            msg.comparedScans.scan1.listing_url &&
            msg.comparedScans.scan2.listing_url
          );
          
          if (!hasValidComparison && finalScanData && finalScanData.scan1 && finalScanData.scan2) {
            // Remove the follow-up message temporarily if it exists
            const followUpIndex = processedMessages.findIndex(msg => 
              msg.content && msg.content.includes("Do you have any question")
            );
            const followUpMessage = followUpIndex >= 0 ? processedMessages.splice(followUpIndex, 1)[0] : null;
            
            // Find the original comparison message content if it exists
            const originalComparisonMessage = (chatData.messages || []).find(msg => 
              msg.role === 'assistant' && 
              msg.content && 
              msg.content.trim().length > 0 &&
              (msg.content.includes('Listing A:') || msg.content.includes('Comparative Analysis:') || msg.content.length > 50)
            );
            
            // Extract analysis content from original message
            let analysisContent = "Here's the comparison of the two listings.";
            if (originalComparisonMessage && originalComparisonMessage.content) {
              const analysisStart = originalComparisonMessage.content.indexOf('Comparative Analysis:');
              if (analysisStart !== -1) {
                analysisContent = originalComparisonMessage.content.substring(analysisStart + 'Comparative Analysis:'.length).trim();
              } else {
                // Use the full content if no "Comparative Analysis:" marker
                analysisContent = originalComparisonMessage.content;
              }
            }
            
            // Ensure we have some content
            if (!analysisContent || analysisContent.trim().length === 0) {
              analysisContent = "Here's the comparison of the two listings.";
            }
            
            // Add comparison message FIRST with proper structure
            // This MUST be at the beginning so it displays first
            processedMessages.unshift({
              role: "assistant",
              ...
              comparedScans: {
                scan1: {
                  listing_url: finalScanData.scan1.listing_url,
                  listing_title: finalScanData.scan1.listing_title || finalScanData.scan1.title || 'Listing A'
                },
                scan2: {
                  listing_url: finalScanData.scan2.listing_url,
                  listing_title: finalScanData.scan2.listing_title || finalScanData.scan2.title || 'Listing B'
                }
              },
              timestamp: chatData.chat.created_at || new Date().toISOString()
            });
            
            // Re-add follow-up message at the end
            if (followUpMessage) {
              processedMessages.push(followUpMessage);
            }
          }
          
          // FINAL CHECK: Before setting messages, ensure we have a valid comparison message
          // This is the absolute last chance to prevent blank screen
          const hasComparison = processedMessages.some(msg => {
            if (!msg.isComparison || !msg.comparedScans) return false;
            const scans = msg.comparedScans;
            return scans.scan1 && scans.scan2 && 
                   scans.scan1.listing_url && scans.scan2.listing_url;
          });
          
          // DEBUG: Log comparison check
          console.log('🔍 [DEBUG] hasComparison before final check:', hasComparison);
          console.log('🔍 [DEBUG] processedMessages count:', processedMessages.length);
          console.log('🔍 [DEBUG] processedMessages with isComparison:', processedMessages.filter(m => m.isComparison).length);
          
          if (!hasComparison) {
            console.log('⚠️ [DEBUG] No comparison found, creating one...');
            // Try to get scan data from finalScanData or from fallback data
            let scanDataToUse = finalScanData;
            if (!scanDataToUse && fallbackScan1 && fallbackScan2) {
              scanDataToUse = {
                scan1: fallbackScan1,
                scan2: fallbackScan2
              };
            ...
            if (scanDataToUse && scanDataToUse.scan1 && scanDataToUse.scan2) {
              // Remove any existing follow-up message
              const filteredMessages = processedMessages.filter(msg => 
                !msg.content || !msg.content.includes("Do you have any question")
              );
              
              // Create comparison message with guaranteed structure
              const comparisonMsg = {
                role: "assistant",
                content: "Here's the comparison of the two listings.",
                isComparison: true,
                comparedScans: {
                  scan1: {
                    listing_url: scanDataToUse.scan1.listing_url || scanDataToUse.scan1.url || scanDataToUse.scan1.link || '',
                    listing_title: scanDataToUse.scan1.listing_title || scanDataToUse.scan1.title || 'Listing A'
                  },
                  scan2: {
                    listing_url: scanDataToUse.scan2.listing_url || scanDataToUse.scan2.url || scanDataToUse.scan2.link || '',
                    listing_title: scanDataToUse.scan2.listing_title || scanDataToUse.scan2.title || 'Listing B'
                  }
                },
                timestamp: chatData.chat.created_at || new Date().toISOString()
              };
              
              // Add comparison message first, then other messages, then follow-up
              processedMessages = [comparisonMsg, ...filteredMessages, {
                role: "assistant",
                content: "Do you have any question...",
                timestamp: new Date().toISOString()
              }];
              
              console.log('✅ [DEBUG] Comparison message created and added to processedMessages');
            } else {
              console.log('❌ [DEBUG] No scanDataToUse available to create comparison');
            }
          } else {
            console.log('✅ [DEBUG] Comparison message already exists in processedMessages');
          }
          
          // DEBUG: Final check before setting messages
          const finalCheck = processedMessages.some(msg => {
            if (!msg.isComparison || !msg.comparedScans) return false;
            const scans = msg.comparedScans;
            return scans.scan1 && scans.scan2 && 
                   scans.scan1.listing_url && scans.scan2.listing_url;
          });
          console.log('🔍 [DEBUG] Final check - hasComparison:', finalCheck);
          console.log('🔍 [DEBUG] Total messages to set:', processedMessages.length);
          console.log('🔍 [DEBUG] Messages breakdown:', {
            comparison: processedMessages.filter(m => m.isComparison).length,
            followUp: processedMessages.filter(m => m.content && m.content.includes("Do you have any question")).length,
            user: processedMessages.filter(m => m.role === 'user').length,
            assistant: processedMessages.filter(m => m.role === 'assistant').length
          });
          
          // Set messages - FORCE create comparison if still missing
          if (!finalCheck && finalScanData && finalScanData.scan1 && finalScanData.scan2) {
            console.log('🚨 [DEBUG] EMERGENCY: Creating comparison message in setMessages');
            const emergencyComparison = {
              ...
              comparedScans: {
                scan1: {
                  listing_url: finalScanData.scan1.listing_url || finalScanData.scan1.url || finalScanData.scan1.link || '',
                  listing_title: finalScanData.scan1.listing_title || finalScanData.scan1.title || 'Listing A'
                },
                scan2: {
                  listing_url: finalScanData.scan2.listing_url || finalScanData.scan2.url || finalScanData.scan2.link || '',
                  listing_title: finalScanData.scan2.listing_title || finalScanData.scan2.title || 'Listing B'
                }
              },
              timestamp: chatData.chat.created_at || new Date().toISOString()
            ...
            const withoutFollowUp = processedMessages.filter(msg => 
              !msg.content || !msg.content.includes("Do you have any question")
            );
            
            processedMessages = [emergencyComparison, ...withoutFollowUp, {
              role: "assistant",
              content: "Do you have any question...",
              timestamp: new Date().toISOString()
            }];
            
            console.log('✅ [DEBUG] Emergency comparison message created');
          }
          ...
          // Set messages
          console.log('📤 [DEBUG] Setting messages, count:', processedMessages.length);
          setMessages(processedMessages);
          
          // Loading state will be cleared by useEffect when messages are confirmed to be rendered
        ...
        console.log('📱 [DEBUG] Using local comparison data:', {
          id: compare.id,
          hasScan1: !!compare.scan1,
          hasScan2: !!compare.scan2,
          hasFirstListing: !!compare.firstListing,
          hasSecondListing: !!compare.secondListing,
          hasAnswer: !!compare.answer
        });
        
        // Map scan1/scan2 to firstListing/secondListing if needed (from HistoryScreen format)
        const firstListing = compare.firstListing || compare.scan1;
        const secondListing = compare.secondListing || compare.scan2;
        ...
        // Load existing chat history from stored messages if available
        if (compare.messages && Array.isArray(compare.messages) && compare.messages.length > 0) {
          console.log('📱 [DEBUG] Loading chat history for local compare:', compare.messages.length, 'messages');
          
          // Process stored messages
          compare.messages.forEach((msg, index) => {
            // Filter out "Rating" messages (only assistant messages, not user questions)
            if (msg.role === 'assistant' && msg.content && msg.content.trim().toLowerCase() === 'rating') {
            ...
            // If this is the first assistant message (index 1) and it's the comparison result
            if (msg.role === 'assistant' && index === 1 && firstListing && secondListing) {
              message.isComparison = true;
              message.comparedScans = {
                scan1: {
                  listing_url: firstListing.listing_url || firstListing.url || firstListing.link || '',
                  listing_title: firstListing.listing_title || firstListing.title || 'Listing A'
                },
                scan2: {
                  listing_url: secondListing.listing_url || secondListing.url || secondListing.link || '',
                  listing_title: secondListing.listing_title || secondListing.title || 'Listing B'
                }
              };
            }
          ...
          // Only add follow-up if last message is not already a follow-up
          const lastMessage = localMessages[localMessages.length - 1];
          if (!lastMessage || !lastMessage.content || !lastMessage.content.includes("Do you have any question")) {
            localMessages.push({
              role: "assistant",
              content: "Do you have any question...",
              timestamp: new Date().toISOString()
          ...
          // No stored messages - create initial messages (backward compatibility)
          // Add user message
          if (firstListing && secondListing) {
            localMessages.push({
              ...
              messageType: "compare",
              timestamp: compare.createdAt || compare.created_at
            });
          }
          
          // Add assistant message with comparison result
          // CRITICAL: Ensure comparison message has proper structure
          if (compare.answer && firstListing && secondListing) {
            const comparisonMessage = {
              role: "assistant",
            ...
            console.log('✅ [DEBUG] Created local comparison message:', {
              hasContent: !!comparisonMessage.content,
              hasComparedScans: !!comparisonMessage.comparedScans,
              scan1Url: comparisonMessage.comparedScans.scan1.listing_url,
              scan2Url: comparisonMessage.comparedScans.scan2.listing_url
            });
            
            localMessages.push(comparisonMessage);
          } else {
            console.log('⚠️ [DEBUG] Missing data for local comparison:', {
              hasAnswer: !!compare.answer,
              hasFirstListing: !!firstListing,
              hasSecondListing: !!secondListing
            });
          }
        ...
        // Final verification
        const hasValidComparison = localMessages.some(msg => 
          msg.isComparison && 
          msg.comparedScans && 
          msg.comparedScans.scan1 && 
          msg.comparedScans.scan2 &&
        ...
        console.log('🔍 [DEBUG] Local messages before setting:', {
          total: localMessages.length,
          hasValidComparison: hasValidComparison,
          comparisonIndex: localMessages.findIndex(m => m.isComparison)
        });
        ...
        // Clear loading state after messages are set (use setTimeout to ensure state update is processed)
        setTimeout(() => {
          setLoadingCompare(false);
        }, 150);
      }
  ...
  const loadCompareWithMessages = React.useCallback(async (compare, chatMessages) => {
    try {
      // Set loading state to show loading indicator instead of blank screen
      // Only set if we're not delegating to handleRecentComparePress
      ...
      if (!chatMessages || chatMessages.length === 0) {
        // If no messages provided, fall back to handleRecentComparePress (it will handle loading state)
  ...
  // Clear loading state only when messages are actually loaded and rendered
  useEffect(() => {
    if (loadingCompare && messages.length > 0) {
      // DEBUG: Log messages state
      console.log('🔍 [DEBUG useEffect] Messages received:', messages.length);
      console.log('🔍 [DEBUG useEffect] Messages with isComparison:', messages.filter(m => m.isComparison).length);
      
      // Check if we have a valid comparison message with scan data
      const hasValidComparison = messages.some(msg => {
        if (!msg.isComparison || !msg.comparedScans) return false;
        const scans = msg.comparedScans;
        const isValid = scans.scan1 && scans.scan2 && 
               scans.scan1.listing_url && scans.scan2.listing_url;
        if (isValid) {
          console.log('✅ [DEBUG useEffect] Found valid comparison message:', {
            scan1Url: scans.scan1.listing_url,
            scan2Url: scans.scan2.listing_url
          });
        }
        return isValid;
      });
      
      console.log('🔍 [DEBUG useEffect] hasValidComparison:', hasValidComparison);
      
      // Only clear loading if we have a valid comparison message
      // This ensures the comparison UI is ready to display
      if (hasValidComparison) {
        console.log('✅ [DEBUG useEffect] Clearing loading state (has comparison)');
        // Use a delay to ensure React has rendered the messages before clearing loading state
        const timer = setTimeout(() => {
          setLoadingCompare(false);
        }, 400);
        return () => clearTimeout(timer);
      } else {
        console.log('⚠️ [DEBUG useEffect] No valid comparison, waiting...');
        // If we have messages but no valid comparison, wait a bit longer
        // This gives time for the comparison message to be created
        const timer = setTimeout(() => {
          // Check again if comparison message exists now
          const stillNoComparison = !messages.some(msg => {
            if (!msg.isComparison || !msg.comparedScans) return false;
            const scans = msg.comparedScans;
            return scans.scan1 && scans.scan2 && 
                   scans.scan1.listing_url && scans.scan2.listing_url;
          ...
          console.log('🔍 [DEBUG useEffect] After wait, stillNoComparison:', stillNoComparison);
          
          // Only clear if we still don't have comparison after waiting
          // This prevents infinite loading but allows time for message creation
          if (stillNoComparison) {
            console.log('⚠️ [DEBUG useEffect] Clearing loading despite no comparison (timeout)');
            setLoadingCompare(false);
          } else {
            console.log('✅ [DEBUG useEffect] Found comparison after wait, clearing loading');
            setLoadingCompare(false);
          }
        }, 500);
        return () => clearTimeout(timer);
  ...
  useEffect(() => {
    if (route?.params?.loadCompare) {
      ...
      // Set loading state immediately to show loading indicator instead of blank screen
      setLoadingCompare(true);
      // Clear messages to prevent showing old content during loading
      setMessages([]);
      ...
      const loadCompareWithFreshMessages = async () => {
        console.log('🔵 [DEBUG useEffect] loadCompare triggered with:', {
          id: compareToLoad.id,
          source: compareToLoad.source,
          title: compareToLoad.title,
          hasChatMessages: !!compareToLoad.chatMessages,
          chatMessagesCount: compareToLoad.chatMessages?.length || 0
        });
        
        // For local compares, we need to get the full data from local storage
        let compare = null;
        if (compareToLoad.source === 'local') {
          console.log('🔵 [DEBUG useEffect] Loading local compare from storage');
          try {
            const storageKey = getUserStorageKey('local_comparisons');
            const localComparisons = await AsyncStorage.getItem(storageKey);
            if (localComparisons) {
              const parsed = JSON.parse(localComparisons);
              const localCompare = parsed.find(c => c.id === compareToLoad.id);
              if (localCompare) {
                // Map local compare format to what handleRecentComparePress expects
                compare = {
                  id: localCompare.id,
                  chatId: localCompare.id,
                  title: localCompare.title,
                  source: 'local',
                  firstListing: localCompare.firstListing,
                  secondListing: localCompare.secondListing,
                  answer: localCompare.answer,
                  question: localCompare.question,
                  createdAt: localCompare.timestamp || localCompare.createdAt,
                  // Include stored messages for chat history
                  messages: localCompare.messages || []
                };
                console.log('✅ [DEBUG useEffect] Found local compare:', {
                  id: compare.id,
                  hasFirstListing: !!compare.firstListing,
                  hasSecondListing: !!compare.secondListing,
                  hasAnswer: !!compare.answer,
                  messagesCount: compare.messages?.length || 0
                });
              }
            }
          } catch (e) {
            console.log('❌ [DEBUG useEffect] Error loading local compare:', e.message);
          }
        }
        
        // If not found in local storage, try recentCompares or create default
        if (!compare) {
          compare = recentCompares.find(c => c.id === compareToLoad.id || c.chatId === compareToLoad.id);
        }
        
        // If still not found, create from compareToLoad (but map scan1/scan2 for local)
        if (!compare) {
          compare = {
            id: compareToLoad.id,
            chatId: compareToLoad.id,
          ...
          // For local compares passed from HistoryScreen, map scan1/scan2
          if (compareToLoad.source === 'local' && compareToLoad.scan1 && compareToLoad.scan2) {
            compare.firstListing = compareToLoad.scan1;
            compare.secondListing = compareToLoad.scan2;
            // Try to get answer from local storage
            try {
              const storageKey = getUserStorageKey('local_comparisons');
            const localComparisons = await AsyncStorage.getItem(storageKey);
              if (localComparisons) {
                ...
                const localCompare = parsed.find(c => c.id === compareToLoad.id);
                if (localCompare) {
                  compare.answer = localCompare.answer;
                }
              }
            } catch (e) {
              // Ignore
        ...
        console.log('🔵 [DEBUG useEffect] Final compare object:', {
          id: compare.id,
        ...
        if (compareToLoad.id && compareToLoad.source === 'backend') {
          ...
          // For local compares, always use handleRecentComparePress (it handles local data properly)
          console.log('🔵 [DEBUG useEffect] Using handleRecentComparePress for local compare');
          await handleRecentComparePress(compare);
        }
      ...
      loadCompareWithFreshMessages().catch((error) => {
        // Handle any errors and clear loading state
  ...
  const renderMessage = (message, index) => {
    ...
    // DEBUG: Log comparison message rendering
    if (message.isComparison) {
      console.log('🎨 [DEBUG render] Rendering comparison message:', {
        hasComparedScans: !!message.comparedScans,
        hasScan1: !!message.comparedScans?.scan1,
        hasScan2: !!message.comparedScans?.scan2,
        scan1Url: message.comparedScans?.scan1?.listing_url,
        scan2Url: message.comparedScans?.scan2?.listing_url,
        willRender: isComparison
      });
    }
  ...
  const handleSend = async () => {
    ...
    const filteredMessages = messages.filter(msg => {
      ...
      // Filter out assistant messages with "Rating" content
      return !(msg.content && msg.content.trim().toLowerCase() === 'rating');
    });
    ...
    try {
      ...
      // Check if this is a compare chat - prioritize /compare endpoint like web app
      // Try to get URLs from currentCompareChat first (works for both local and backend compare chats)
      let firstUrl = null;
      let secondUrl = null;
      
      if (currentCompareChat && currentCompareChat.scan1 && currentCompareChat.scan2) {
        // Get URLs from currentCompareChat (works for both local and backend compare chats)
      ...
      // If URLs not found, try to extract from messages
      if ((!firstUrl || !secondUrl) && messages && messages.length > 0) {
        const comparisonMessage = messages.find(msg => 
          ...
          (msg.isComparison || msg.comparedScans || 
           (msg.content && (msg.content.includes('Listing A:') || msg.content.includes('Comparative Analysis:'))))
        );
      ...
      // If we have both URLs, use /compare endpoint directly (same as web app for compare chats)
      if (firstUrl && secondUrl) {
        // Use /compare endpoint for comparison questions (same as web frontend)
        try {
          const { data, error } = await apiClient.compareListings(
            firstUrl,
            secondUrl,
          ...
          if (error) {
            ...
            // Save messages to local storage if it's a local compare
            if (currentCompareChat && currentCompareChat.id && currentCompareChat.id.startsWith('local_')) {
              await saveLocalCompareMessage(currentCompareChat.id, 'user', userQuestion);
              await saveLocalCompareMessage(currentCompareChat.id, 'assistant', data.answer);
            }
          ...
          setMessages(prev => {
            const filtered = prev.filter(msg => !msg.isTyping && !(msg.content && msg.content.trim().toLowerCase() === 'rating'));
            const updated = [...filtered, errorMessage];
            // Sort by timestamp to ensure chronological order
            updated.sort((a, b) => {
              const timeA = new Date(a.timestamp || a.created_at || 0).getTime();
        ...
        try {
          ...
          if (error) {
            ...
            if (isInternalServerError) {
              ...
              if (firstUrl && secondUrl) {
                ...
                return !(msg.content && msg.content.trim().toLowerCase() === 'rating');
                    ...
                    if (currentCompareChat && currentCompareChat.id && currentCompareChat.id.startsWith('local_')) {
                      await saveLocalCompareMessage(currentCompareChat.id, 'user', userQuestion);
                      await saveLocalCompareMessage(currentCompareChat.id, 'assistant', compareData.answer);
                    }
            ...
            setMessages(prev => {
              ...
              const updated = [...filtered, errorMsgObj];
              // Sort by timestamp to ensure chronological order
              updated.sort((a, b) => {
                const timeA = new Date(a.timestamp || a.created_at || 0).getTime();
                const timeB = new Date(b.timestamp || b.created_at || 0).getTime();
            ...
            // Save messages to local storage if it's a local compare
            // (Backend compares are saved to database automatically)
            if (currentCompareChat && currentCompareChat.id && currentCompareChat.id.startsWith('local_')) {
              await saveLocalCompareMessage(currentCompareChat.id, 'user', userQuestion);
              await saveLocalCompareMessage(currentCompareChat.id, 'assistant', data.answer);
            }
        ...
        try {
          ...
          if (error) {
            ...
            const filtered = updatedMessages.filter(msg => {
              // Always show user messages
              if (msg.role === 'user') return true;
              // Filter out assistant messages with "Rating" content
              return !(msg.content && msg.content.trim().toLowerCase() === 'rating');
            });
            const newMessages = [...filtered, errorMsg];
            // Sort by timestamp to ensure chronological order
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {/* Show loading indicator when loading a recent compare - takes full screen space */}
        {loadingCompare ? (
          <View style={styles.loadingContainer}>
            <ActivityIndicator size="large" color="#1e162a" />
        ...
        {(() => {
          ...
          console.log('🎨 [DEBUG render] MessagesArea render check:', {
            messagesLength: messages.length,
            loadingCompare: loadingCompare,
            willRender: messages.length > 0,
            comparisonCount: messages.filter(m => m.isComparison).length
          });
          return null;
        })()}
        {messages.length > 0 && (
