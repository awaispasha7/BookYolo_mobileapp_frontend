...
// Helper function to detect if input is an Expedia URL
...
export default function ScanScreen({ navigation, route }) {
  ...
  const handleScan = async (url) => {
    ...
    try {
      ...
      if (isNotInScope) {
        ...
        errorContent =
          "We couldn't find this listing in our database.This property does not exist in our database yet. Please try another listing from Airbnb, Booking.com, Agoda, or Expedia.";
  ...
  const handleAsk = async (question) => {
    ...
    // Check if user has sufficient balance for questions (requires 0.5 scans) - MATCHING WEB APP
    if (!currentChatId) {
      // For pre-scan questions, no balance check needed
    } else {
      // For questions after scan, check balance first (same as web app)
      if (scanBalance?.remaining !== undefined && scanBalance.remaining < 0.5) {
        setMessages(prev => [...prev, {
          role: "assistant",
    ...
    try {
      if (!currentChatId) {
        ...
        if (error) {
          // Better error handling - check if it's a timeout/abort
        ...
        if (error) {
          ...
          // Check if it's a balance/limit error - pass through as-is
          if (error.includes("limit") || error.includes("balance") || error.includes("402")) {
            throw new Error(error);
          }
          throw new Error(error);
        }
      ...
      // Remove typing indicator and add error message - MATCHING WEB APP FORMAT
      setMessages(prev => {
        // Remove typing indicators
        const filtered = prev.filter(msg => !msg.isTyping);
        return [...filtered, {
          ...
          content: !currentChatId 
            ...
            : `Sorry, I couldn't answer that question. ${e.message || "An error occurred."}`,
      ...
      <View style={styles.blueCard}>
        ...
        {isAlready ? (
          <View pointerEvents="box-none">
            ...
            <View style={styles.blueButtonRow} pointerEvents="box-none">
              <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  messageBubble: {
    ...
    borderRadius: 16,
    padding: 16,
    flexShrink: 1, // Allow the bubble to shrink and wrap text properly
  },
  ...
  messageText: {
    ...
    flexWrap: 'wrap',
    flexShrink: 1, // Allow text to shrink and wrap properly, preventing truncation
  },
  errorText: {
    color: "#dc2626",
  },
