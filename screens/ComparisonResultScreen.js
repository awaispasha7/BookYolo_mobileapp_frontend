...
const ComparisonResultScreen = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Try to get URLs from comparisonData or messages first
      let firstUrl = null;
      let secondUrl = null;
      
      // Get URLs from comparisonData first - handle both data structures
      // Recent compares use scan1/scan2, new compares use firstListing/secondListing
      if (comparisonData?.scan1 && comparisonData?.scan2) {
        // Handle recent compare structure (from HistoryScreen)
        firstUrl = comparisonData.scan1.listing_url || comparisonData.scan1.link || comparisonData.scan1.url;
        secondUrl = comparisonData.scan2.listing_url || comparisonData.scan2.link || comparisonData.scan2.url;
      } else if (comparisonData?.firstListing && comparisonData?.secondListing) {
        // Handle new compare structure
        firstUrl = comparisonData.firstListing.listing_url || comparisonData.firstListing.link || comparisonData.firstListing.url;
        secondUrl = comparisonData.secondListing.listing_url || comparisonData.secondListing.link || comparisonData.secondListing.url;
      }
      ...
      if (firstUrl && secondUrl) {
        ...
        try {
          ...
          if (compareError) {
            ...
            const newMessages = [...updatedMessages, { 
              role: "assistant", 
              content: `Sorry, I encountered an error: ${compareError}. Please try again.`,
              isError: true
            }];
            setMessages(newMessages);
          } else if (compareData && compareData.answer) {
            // Use /compare endpoint response
            const newMessages = [...updatedMessages, { 
              role: "assistant", 
          ...
          setTimeout(async () => {
            // Handle both data structures (scan1/scan2 for recent compares, firstListing/secondListing for new compares)
            const firstListing = comparisonData?.scan1 || comparisonData?.firstListing;
            const secondListing = comparisonData?.scan2 || comparisonData?.secondListing;
        ...
        // Use backend chat system for non-compare chats (scan chats)
      ...
      const firstListing = comparisonData?.scan1 || comparisonData?.firstListing;
      const secondListing = comparisonData?.scan2 || comparisonData?.secondListing;
      const comparisonDataForFallback = {
        firstListing: firstListing,
        secondListing: secondListing,
        answer: comparisonResult.answer,
        question: comparisonData?.question
      };
      const fallbackResponse = generateAIResponse(userQuestion, comparisonDataForFallback);
