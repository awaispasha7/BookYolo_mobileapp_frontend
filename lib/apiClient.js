...
class ApiClient {
  ...
  // Optimized for polling - uses shorter timeout for faster response
  async getScanById(scanId) {
    // Use shorter timeout for polling requests (10s instead of 30s)
    // This is called frequently during polling, so faster timeout = better UX
    const originalTimeout = this.timeout;
    this.timeout = 10000; // 10 seconds for polling requests
    
    try {
      const result = await this.request(`/scan/${scanId}`);
      return result;
    } finally {
